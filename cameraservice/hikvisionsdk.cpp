...
namespace vimo::display
{
	...
	//发送取图信号
	void HikvisionSDK::emitSignaltop(const cv::Mat & image)
	{
		Mat imageshow = image.clone();
		Mat imagesave = image.clone();
		emit imageshowsignaltop(imageshow);
		emit imagesaveignaltop(imagesave);
		emit imageoperationignaltop(image);
	}
	void HikvisionSDK::emitSignalin(const cv::Mat & image)
	{
		Mat imageshow = image.clone();
		Mat imagesave = image.clone();
		emit imageshowsignalin(imageshow);
		emit imagesaveignalin(imagesave);
		emit imageoperationignalin(image);
	}
	void HikvisionSDK::emitSignalside1(const cv::Mat & image)
	{
		Mat imageshow = image.clone();
		Mat imagesave = image.clone();
		emit imageshowsignalside1(imageshow);
		emit imagesaveignalside1(imagesave);
		emit imageoperationignalside1(image);
	}
	void HikvisionSDK::emitSignalside2(const cv::Mat & image)
	{
		Mat imageshow = image.clone();
		Mat imagesave = image.clone();
		emit imageshowsignalside2(imageshow);
		emit imagesaveignalside2(imagesave);
		emit imageoperationignalside2(image);
	}
	void HikvisionSDK::emitSignalside3(const cv::Mat & image)
	{
		Mat imageshow = image.clone();
		Mat imagesave = image.clone();
		emit imageshowsignalside3(imageshow);
		emit imagesaveignalside3(imagesave);
		emit imageoperationignalside3(image);
	}
	void HikvisionSDK::emitSignalside4(const cv::Mat & image)
	{
		Mat imageshow = image.clone();
		Mat imagesave = image.clone();
		emit imageshowsignalside4(imageshow);
		emit imagesaveignalside4(imagesave);
		emit imageoperationignalside4(image);
	}
}
